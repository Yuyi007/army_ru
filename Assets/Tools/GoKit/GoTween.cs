...
public class GoTween : AbstractGoTween
{
    ...
    // the target of the tweens
    public float delay { get; private set; }
    // delay before starting the actual animations
    private float _elapsedDelay;
    // total time delayed
    private bool _delayComplete;
    // once we complete the delay this gets set so we can reverse and play properly for the future
    public bool isFrom { get; private set; }
    // a value of true will make this a "from" tween
    ...
    public GoTween(object target, float duration, GoTweenConfig config)
    {
        initialize(target, duration, config, null);
    }

    /// <summary>
    /// initializes a new instance and sets up the details according to the config parameter
    /// </summary>
    public GoTween(object target, float duration, GoTweenConfig config, Action<AbstractGoTween> onComplete)
    {
        initialize(target, duration, config, onComplete);
    }
    ...
    public override bool update(float deltaTime)
    {
        ...
        var length = _tweenPropertyList.Count;
    ...
    {
        if ((this.target as UnityEngine.Object) == null)
        {
            this.destroy();
            return;
        }
    ...
    // 2016-8-2 lt: facilitate to reuse tweens
    public void clearDidInitFlag()
    {
        _didInit = false;
    }

    // 2016-8-2 lt: facilitate to reuse tweens
    public override void prepareAllTweenProperties()
    {
        var length = _tweenPropertyList.Count;
        for (var i = 0; i < length; ++i)
        {
