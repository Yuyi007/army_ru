...
public class AssetHunterMainWindow : EditorWindow, ISerializationCallbackReceiver
{
    ...
    void OnGUI()
    {
        ...
        if (m_BuildLogLoaded)
        {
            ...
            //else if (m_WindowState == AssetHunterWindowState.UnusedScripts)
    ...
    private void OnShowDefaultGUI()
    {
        ...
        /*if (GUILayout.Button(AssetHunterWindowState.UnusedScripts.ToString(), GUILayout.MinWidth(btnMinWidth)))
        {
            //Get added scripts
    ...
    private void OnUnusedScriptsUIUpdate()
    {
       if (m_UsedScriptList == null || m_UsedScriptList.Count <= 0)
        ...
        GUILayout.Label("List of unused scripts");
        showUnusedScriptsUI();
    ...
    private void showUnusedScriptsUI()
    {
        ...
        GUILayout.Label("USEDSCRIPTS");
        foreach (Type t in m_UsedScriptList)
        {
            ...
            if (GUILayout.Button(t.ToString()))
            {
                ...
                {
                    if (m.GetClass() != null && m.GetClass().Equals(t))
                    {
                        script = m;
                    }
                }
