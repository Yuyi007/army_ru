...
namespace HeurekaGames
{
    internal class AssetHunterHelper
    {
        ...
        /*public static List<Type> GetScriptAssembly()
        {
            //Get Assembly
        ...
        //Loop scenes added to buildsettings and find their script dependencies
        /*public static List<Type> GetScriptLevelDependencies(EditorBuildSettingsScene[] ebsScenes)
        {
            ...
            string[] levelDependencies = AssetDatabase.GetDependencies(levelPaths);

            //TODO Loop resources as well to find scripts in those
            //Loop scene dependencies
            foreach (string level in levelDependencies)
            {
                //Is it a script?
                MonoScript ms = AssetDatabase.LoadAssetAtPath<MonoScript>(level);
                if (ms != null)
                                ...
                                && val.Operand.ToString().Contains("AddComponent")))
                            {
                                ...
                                if (genInstanceRef.HasGenericArguments)
                                {
                                    foreach (Mono.Cecil.TypeReference typeRef in genInstanceRef.GenericArguments)
                                    {
                                        ...
                                        Debug.Log("Generic Param: " + typeRef.Name);
                                    }
                                }
                            }
                    }
        ...
        /*public static List<Type> GetAddedComponents()
        {
            ...
            foreach (Mono.Cecil.TypeDefinition td in scriptModuleDef.Types)
            {
                referencedTypes.Add(Type.GetType(td.FullName + ", " + td.Module.Assembly.FullName));
            }
