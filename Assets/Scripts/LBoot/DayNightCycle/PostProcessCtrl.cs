...
[ExecuteInEditMode]
public class PostProcessCtrl : MonoBehaviour, CtrlClassBase
{
    ...
    //////////////////////////////////////////////////////////////////////
    /// Bloom Optimized Ctrl
    [Header("Bloom Optimized Ctrl")]
    public bool boBloomOptEnable = false;
    [Range(0.0f, 1.5f)]
    public float boThreshold = 0.0f;
    [Range(0.0f, 2.5f)]
    public float boIntensity = 0.0f;
    [Range(0.25f, 5.5f)]
    public float boBlurSize = 0.25f;
    //////////////////////////////////////////////////////////////////////
    ...
    void RefreshPostProcessParams()
    {
        ...
        RefreshBloomAndGlare();
    }

    void RefreshBloomOpt()
    {
        var bloom = watchCam.GetComponent<FVBloomSelectiveStencil>();
        if (bloom)
        {
			bloom.SetEnabled (boBloomOptEnable);
			bool bloomEnabled = bloom.IsEnabled ();
			if (bloomEnabled)
            {
                bloom.threshold = boThreshold;
                bloom.intensity = boIntensity;
                bloom.blurSize = boBlurSize;
            }
        }
    }
